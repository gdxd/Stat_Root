# Stat_Root

Example notebooks for 
* basic statistic distributions
* ROOT usage
* Random numbers


In order to use ROOT in your notebooks in jupyter at [LMU jupyterhub](https://jupyter.physik.uni-muenchen.de) you have to do following setup steps (in terminal/shell) once:
* to use ROOT as module in python
  ```
  echo "module load root/6.32.02" > ~/jupyterhub_environment.sh
  ```
* to use C++ ROOT in jupyter notebook
  ```
  module load python/3.11-2023.09
  module load root/6.32.02
  jupyter kernelspec install --user $ROOTSYS/etc/notebook/kernels/root
  echo "module load root/6.32.02" > ~/jupyterhub_environment.sh
  ```

In both cases select environment  **`python/3.11-2023.09`** when you start on **jupyterhub**
